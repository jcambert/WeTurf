...
public class LoadOutputFolderIntoDbHandler
    : AbpHandler.With<LoadOutputFolderIntoDbQuery, LoadOutputFolderIntoDbResponse>
{
    private readonly List<Error> _errors = new();

    public LoadOutputFolderIntoDbHandler(IAbpLazyServiceProvider serviceProvider)
    ...
    )
    {
        ...
        try
        {
            if (Directory.Exists(request.Folder))
            {
                ...
                foreach (var file in files)
                {
                    ...
                    else
                    {
                        ...
                        {
                            var q = new LoadResultatIntoDbQuery()
                            {
                                Filename = file,
                                Rename = false
                            };
                        ...
                        {
                            var q = new LoadPredictedIntoDbQuery()
                            {
                                Filename = file,
                                Rename = false
                            };
                ...
                if (_errors.Any())
                {
                    ...
                    return Result.ValidWithFailure(
                        new LoadOutputFolderIntoDbResponse(),
                        _errors.ToArray()
                    );
                }
            ...
            return Result.Failure<LoadOutputFolderIntoDbResponse>(
                new Error($"{request.Folder} n'existe pas")
            );
        }
        catch (Exception ex)
        {
