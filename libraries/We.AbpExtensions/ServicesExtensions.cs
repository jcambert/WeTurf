...
public static class ServicesExtensions
{
    ...
    public static IObjectMapper ObjectMapper(this IAbpLazyServiceProvider provider, Type ObjectMapperContext=null) 
        => provider.LazyGetService<IObjectMapper>(p =>
        ObjectMapperContext == null
            ? p.GetRequiredService<IObjectMapper>()
            : (IObjectMapper)p.GetRequiredService(typeof(IObjectMapper<>).MakeGenericType(ObjectMapperContext)));
    ...
    public static ILogger Logger<T>(this IAbpLazyServiceProvider provider)
        => provider.LazyGetService<ILogger>(p => provider.LoggerFactory()?.CreateLogger(typeof(T).FullName) ?? NullLogger.Instance);
}