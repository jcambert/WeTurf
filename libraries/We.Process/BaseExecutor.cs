...
public abstract class BaseExecutor : IExecutor
{
    ...
    protected virtual string GetProcessFilename() => "cmd.exe";

    protected virtual string GetArguments(params ICommand[] commands) => string.Empty;
    ...
    )
    {
        ...
        Func<
            ...
            ICommand[],
            Task<Result>
        > fn = UseReactiveOutput ? ExecuteWithReactive : ExecuteWithoutReactive;
        return fn(
            psi,
            cancellationToken,
            GetFilterCommand(),
            GetBeforeSendCommands(),
            GetAfterSendCommands(),
            commands
        );
    }
        ...
        if (proc.Start())
        {
            ...
            var j = Observable
                .Using(
                    ...
                    reader =>
                        Observable
                            .FromAsync(reader.ReadLineAsync, Scheduler)
                            .Repeat()
                            .TakeWhile(x => x != null)
                )
                .ObserveOn(Scheduler);
