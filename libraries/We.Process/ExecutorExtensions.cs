...
public static class ExecutorExtensions
{
    ...
    public static IServiceCollection UsePythonExecutor(
        this IServiceCollection services,
        Action<PythonExecutorOptions> opt
    )
    {
        services.UseExecutor();
        services.Configure(opt);
        services.AddTransient<IAnaconda, Anaconda>(
            sp =>
            {
                var di = sp.GetRequiredService<DriveService>();
                var o = sp.GetRequiredService<IOptions<PythonExecutorOptions>>().Value;
                var anaconda = new Anaconda(di)
                {
                    BasePath = o.AnacondBasePath,
        ...
        services.AddTransient<IAnacondaActivationCommand, AnacondaActivationCommand>(
            sp => new AnacondaActivationCommand(sp)
        );
        services.AddTransient<IAnacondaDeactivationCommand, AnacondaDeactivationCommand>(
            sp => new AnacondaDeactivationCommand(sp)
        );
    ...
    public static IServiceCollection UseCommandExecutor(
        this IServiceCollection services,
        Action<ExecutorOptions> opt
    )
    {
        services.UseExecutor();
        services.Configure(opt);
